...
function renderPublishingPage(page) {
  ...
  return `<main class="content publishing-page"><article><header class="publishing-header"><p class="publishing-header__eyebrow">${eyebrow}</p><h2 class="publishing-header__title">${escapeHtml(page.heading)}</h2><p class="publishing-header__lede">${escapeHtml(page.lede)}</p><p class="publishing-header__updated">Updated ${escapeHtml(page.updatedDate)}</p></header>${renderToolPlaceholder(page)}${page.sections
    .map(renderSection)
